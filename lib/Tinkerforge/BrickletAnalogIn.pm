...
=cut
    ...
    my $self :shared = shared_clone({super => shared_clone(Tinkerforge::Device->new($uid, $ipcon)),
                                     ...
                                     response_expected => shared_clone({&FUNCTION_GET_VOLTAGE => Tinkerforge::Device->RESPONSE_EXPECTED_ALWAYS_TRUE,
                                                                        ...
                                                                        &FUNCTION_SET_RANGE => Tinkerforge::Device->RESPONSE_EXPECTED_FALSE,
                                                                        &FUNCTION_GET_RANGE => Tinkerforge::Device->RESPONSE_EXPECTED_ALWAYS_TRUE,
...
=cut
    ...
    $self->{super}->send_request($self, &FUNCTION_SET_RANGE, [$range], 'C', '');
}
...
=cut
    ...
    return $self->{super}->send_request($self, &FUNCTION_GET_RANGE, [], '', 'C');
}
...
=cut
    ...
    $self->{super}->send_request($self, &FUNCTION_SET_AVERAGING, [$average], 'C', '');
}
...
=cut
    ...
    return $self->{super}->send_request($self, &FUNCTION_GET_AVERAGING, [], '', 'C');
}
